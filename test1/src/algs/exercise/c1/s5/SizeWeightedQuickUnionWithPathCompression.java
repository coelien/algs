...
public class SizeWeightedQuickUnionWithPathCompression {
	private int[] array;
	private int size;
	private int count;
	private int[] weight;
	public SizeWeightedQuickUnionWithPathCompression(int n) {
		array = new int [n];
		weight = new int [n];
	...
	public static void main(String[] args) {
		...
		In in = new In(args[0]);
		SizeWeightedQuickUnionWithPathCompression q = new SizeWeightedQuickUnionWithPathCompression(in.readInt());
		while(!in.isEmpty()) {
			int m = in.readInt();
			int n = in.readInt();
